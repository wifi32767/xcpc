...
### bitset
```c++
bitset<10> b1; // 10位的bitset
bitset<10> b2(1010101010); // 10位的bitset，初始值是1010101010
bitset<10> b3(b2); // 10位的bitset，初始值是b2的值
b1.set(3); // 全部置1
b1.reset(3); // 全部置0
b1.flip(3); // 翻转
b1.test(3); // 测试位是否为1
...
// 双模字符串哈希
ll base[2] = {29, 31};
ll hashmod[2] = {(ll)(1e9 + 9), (ll)(998244353)};
struct Hash{
    int ls;
    ll hsh[2][N];
    ll pwMod[2][N];
...
$height[i]$表示$sa[i]$和$sa[i - 1]$的最长公共前缀，$height[1] = 0$  
应用：  
- 两子串最长公共前缀
...
```c++
struct SAM{
	int link[MAX << 1], len[MAX << 1], tree[MAX << 1][26];
	int last, size;
	    ...
	    while (p != -1 and not tree[p][c]){
      	...
      	if (p == -1) link[cur] = 0;
	        ...
	        if (len[q] == len[p] + 1) link[cur] = q;
	          	...
	          	len[copy] = len[p] + 1;
	          	link[copy] = link[q];
	          	for (int i = 0; i < 26; ++ i) tree[copy][i] = tree[q][i];
	          	while (p != -1 and tree[p][c] == q){
	            	tree[p][c] = copy;
	            	p = link[p];
	          	}
	...
	void init(){
		for (int i = 1; i <= size; i ++) parent[link[i]].push_back(i);
	}
};
```
### 扩展KMP(Z函数)
...
struct TRIE{
    ...
    int find(int x){
        ...
        for (int i = 30; i >= 0; i -- ){
            res <<= 1;
            int c = (x >> i) & 1;
            if (tree[cur][c ^ 1]) cur = tree[cur][c ^ 1], res ++;
            else cur = tree[cur][c];
        }
        return res;
    }
...
\end{matrix}
$$
有$$ans * base = \begin{matrix}
a_1 * b_{11} + a_2 * b_{21} + a_3 * b_{31} +... & a_1 * b_{12} + ... & ... \\ 
... & ... & ...
\end{matrix}$$
```c++
struct grid{
...
//还是要预处理规模在mod以内的组合数
ll lucas(ll m, ll n){
    return n == 0 ? 1 % mod : lucas(m / mod, n / mod) * \
    cal(m % mod, n % mod) % mod;
}
```
### 拓展欧拉定理
...
```c++
vector<int> graph[MAX];
int sz[MAX], dep[MAX], fa[MAX], son[MAX], top[MAX];
int l[MAX], r[MAX], cnt;
void dfs1(int cur, int pre){
    ...
    for (int nex: graph[cur]){
        ...
        dfs1(nex, cur);
        sz[cur] += sz[nex];
        if (sz[nex] > sz[son[cur]]) son[cur] = nex;
    }
}
void dfs2(int cur, int pre){
    ...
    for (int nex: graph[cur]){
        if (nex == pre or nex == son[cur]) continue;
        top[nex] = nex;
        dfs2(nex, cur);
    }
    r[cur] = cnt;
...
### 快读快写
```c++
//普通快读更多用于int128，以及对一个超大数要边读入边取模的情况
ll read(){
    ll x = 0, f = 1;
    char ch = getchar();
    while (ch < '0' || ch > '9'){
...
int main() {
    ...
    while (true){
        ...
        system("my.exe < test.in > my.out");  // 获取程序1输出
...
for (int i = 0; i < n; ++ i){
    ...
    for (int j = 0; j < v[i]; ++ j){
        ...
        for (int k = j; k <= m; k += v[i]){
            while (head <= tail && k - q[head] > s[i] * v[i]){
                head ++;
            }
            while (head <= tail && g[q[tail]] + (k - q[tail]) / v[i] * w[i] <= g[k]){
                tail --;
            }
            tail ++;
            q[tail] = k;
...
3. 球不同，盒同，不允许空箱子  
$dp[n][m]$ 表示 $n$ 个球放在 $m$ 个盒子，有多少种情况，记作 $S[n][m]$  
$dp[n][m] = m \cdot dp[n-1][m] + dp[n-1][m-1]$  
初始化：   
$dp[i][i] = 1 \quad i \geq 1$  
$dp[i][1] = 1 \quad i \geq 1$  
$dp[i][j] = 0 \quad i < j$  

4. 球不同，盒同，允许空箱子  
$dp[n][m]$ 表示 $n$ 个球放在 $m$ 个盒子，有多少种情况  
$dp[n][m] = S[n][1] + S[n][2] + S[n][3] + \ldots + S[n][m]$  
初始化：  
$dp[n][1] = 1$  

5. 球不同，盒不同，不允许空箱子  
$dp[m][n]$ 表示 $m$ 个盒子装 $n$ 个球  
$dp[m][n] = S[m][n] \cdot m!$  
...
7. 球同，盒同，不允许空箱子  
$dp[m][n]$ 记作 $F[m][n]$  
$dp[1][n-m] + dp[2][n-m] + \ldots + dp[m][n-m]$  
$dp[m][n] = dp[1][n-m] + dp[2][n-m] + \ldots + dp[m][n-m]$  

8. 球同，盒同，允许空箱子  
$dp[m][n] = F[1][n] + F[2][n] + \ldots + F[m][n]$
## 公式
### 积分
$\int_0^{\frac{\pi}{2}}sinx^ndx=
\begin{cases}
{\frac{n-1}{n}\cdot\frac{n-3}{n-2}\cdots\frac{3}{4}\cdot\frac{1}{2}\cdot\frac{\pi}{2},n为奇数}\\
{\frac{n-1}{n}\cdot\frac{n-3}{n-2}\cdots\frac{4}{5}\cdot\frac{2}{3}\cdot1,n为偶数}
\end{cases}$  
$\int_0^{\frac{\pi}{2}}cosx^ndx=
\begin{cases}{\frac{n-1}{n}\cdot\frac{n-3}{n-2}\cdots\frac{3}{4}\cdot\frac{1}{2}\cdot\frac{\pi}{2},n为奇数}\\
{\frac{n-1}{n}\cdot\frac{n-3}{n-2}\cdots\frac{4}{5}\cdot\frac{2}{3}\cdot1,n为偶数}\end{cases}$  
...
对于每一个 $ x_i - x_j \leq k $ ，建立一条从 $j$ 到 $i$，权值为 $k$ 的有向边。从点 $0$ 处向每一个点连一条权值为 $0$ 的边，然后在图上从 $0$ 开始跑 spfa。如果有负环，那么无解。否则 $dist[i]$ 即为一组解。  
